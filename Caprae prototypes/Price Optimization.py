...
def calculate_optimized_price(extracted_data):
    ...
    expenses = extracted_data.get("COGS", 0) + extracted_data.get("SG&A", 0)
    profit = extracted_data.get("Revenue", 0) - expenses

    # Predict selling price using the model
    company_features = np.array([[extracted_data["Revenue"], expenses, profit]])
